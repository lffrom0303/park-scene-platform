...
export default class SkyBoxs {
  skyboxTypeList = ["daytime", "dusk", "night"];
  constructor(_viewer) {
  ...
  addFog(color = "rgb(9,9,9)", near = 0.01, far = 50) {
    this._viewer.scene.fog = new THREE.Fog(new THREE.Color(color), near, far);
  }
   ...
   */
  addSkybox(index = 0) {
    const path = `resources/skybox/${this.skyboxTypeList[index]}/`; // 设置路径
  ...
  setSkybox(path, format = ".jpg") {
    ...
    this._viewer.scene.background = cubeTexture;
  }
}
